...
class YouTubeAPI:
    ...
    async def get_related(
        self, title: str, current_vidid: str, video: bool = False
    ) -> dict | None:
        """Fetch a related song (different from current) for autoplay."""
        client = await self.get_client()
        params = {"query": title or "", "limit": 10}
        if API_KEY:
            params["api_key"] = API_KEY
        try:
            response = await client.get(f"{API_URL}/search", params=params)
            if response.status_code == 200:
                ...
                choices = [
                    r
                    for r in results
                    if r.get("id") != current_vidid
                    and r.get("type", "video") == "video"
                ]
                if not choices:
                    choices = [r for r in results if r.get("id") != current_vidid]
                if choices:
                    r = random.choice(choices)
                    return {
                        "id": r.get("id"),
                        "title": r.get("title", "Unknown"),
                        "thumb": (r.get("thumbnails") or [{}])[-1]
                        .get("url", "")
                        .split("?")[0],
