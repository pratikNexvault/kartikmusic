...
from AloneMusic.utils.database import (add_active_chat, add_active_video_chat,
                                       get_autoplay, get_lang, get_loop,
            ...
            if not check:
                ...
                try:
                    if await get_autoplay(chat_id):
                        _v = True if str(popped.get("streamtype")) == "video" else False
                        rel = await YouTube.get_related(
                            popped.get("title"), popped.get("vidid"), video=_v
                        )
                        if rel:
                            item = dict(popped)
                            item["file"] = f"vid_{rel['id']}"
                            item["vidid"] = rel["id"]
                            item["title"] = rel["title"]
                            item["by"] = "Autoplay"
                            item["old_dur"] = None
                            item["old_second"] = None
                            item["played"] = 0
                            item["speed"] = 1.0
                            item["speed_path"] = None
                            item["dur"] = rel.get("duration", "0:00")
                            db[chat_id] = [item]
                            check = db[chat_id]
                            auto = True
                ...
                if not auto:
                    try:
                        ...
                        await app.send_message(
                            chat_id,
                            "**🎵 𝐓ʜᴇ 𝐐ᴜᴇᴜᴇ 𝐇ᴀs 𝐅ɪɴɪsʜᴇᴅ. 𝐔sᴇ /play 𝐓ᴏ 𝐀ᴅᴅ 𝐌ᴏʀᴇ 𝐒𝐎ɴɢs!!**",
